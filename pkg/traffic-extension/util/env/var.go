...
func RegisterVar(v Var) {
	...
	if old, ok := allVars[v.Name]; ok {
		...
		if old.Description != v.Description || old.DefaultValue != v.DefaultValue || old.Type != v.Type || old.Deprecated != v.Deprecated || old.Hidden != v.Hidden {
			klog.Warningf("The environment variable %s was registered multiple times using different metadata: %v, %v", v.Name, old, v)
		}
	} else {
		allVars[v.Name] = v
	}
...
func (v FloatVar) Lookup() (float64, bool) {
	...
	if err != nil {
		klog.Warningf("Invalid environment variable value `%s`, expecting a floating-point value, defaulting to %v", result, v.DefaultValue)
...
func (v GenericVar[T]) Lookup() (T, bool) {
	...
	if err := json.Unmarshal([]byte(result), res); err != nil {
		klog.Warningf("Invalid environment variable value `%s` defaulting to %v: %v", result, v.DefaultValue, err)
